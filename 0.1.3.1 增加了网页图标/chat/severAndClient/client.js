...
function ajax(method, url, val) {  // 方法，路径，传送数据
    ...
    xhr.onreadystatechange = function() {
        ...
        // 删除上一个空白   新增空白
        var childLiNode = ChatPlaceUl.lastChild;
        console.log(ChatPlaceUl.lastChild);
        ChatPlaceUl.removeChild(childLiNode); 
        ChatPlaceUl.appendChild(li);
        var li = document.createElement("li");
        ChatPlaceUl.appendChild(li);
...
chatPlaceInput.addEventListener('keyup', function(e) {
    if(chatPlaceInput.value.replace(/\s+/g,"")=="" && e.code=="Enter"){
        alert("请勿发送空消息！")
    }
